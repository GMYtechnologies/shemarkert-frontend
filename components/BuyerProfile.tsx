...
export default function BuyerDashboard({ user }: BuyerDashboardProps) {
  ...
  // Handle liking a product - simplified to use localStorage only
  const handleLikeProduct = async (productId: number) => {
    try {
      ...
      const isAlreadyLiked = likedProducts.some((p: any) => p.id === productId);

      if (isAlreadyLiked) {
        // Remove from liked
        const updatedLikes = likedProducts.filter((p: any) => p.id !== productId);
        ...
        alert('Removed from your liked products.');
      } else {
        // Add to liked
        const likedProduct = {
          ...product,
        ...
        const updatedLikes = [...likedProducts, likedProduct];
        localStorage.setItem('likedProducts', JSON.stringify(updatedLikes));
        ...
        alert(`Added "${product.name}" to your liked products!`);
      }
    } catch (error) {
